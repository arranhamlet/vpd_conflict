...
#Death
S_death[, , ] <- if(S[i, j, k] <= 0) 0 else Binomial(S[i, j, k], background_death[i, k])
E_death[, , ] <- if(E[i, j, k] <= 0) 0 else Binomial(E[i, j, k], background_death[i, k])
I_death[, , ] <- if(I[i, j, k] <= 0) 0 else Binomial(I[i, j, k], background_death[i, k])
R_death[, , ] <- if(R[i, j, k] <= 0) 0 else Binomial(R[i, j, k], background_death[i, k])
Is_death[, , ] <- if(Is[i, j, k] <= 0) 0 else Binomial(Is[i, j, k], background_death[i, k] + severe_death_rate)
Rc_death[, , ] <- if(Rc[i, j, k] <= 0) 0 else Binomial(Rc[i, j, k], background_death[i, k])

#S sampling
lambda_S[, , ] <- if(S[i, j, k] <= 0 || lambda[i, j, k] < 0) 0 else Binomial(S[i, j, k], lambda[i, j, k])
waning_R[, , ] <- if(R[i, j, k] <= 0) 0 else Binomial(R[i, j, k], delta)
waning_Rc[, , ] <- if(Rc[i, j, k] <= 0) 0 else Binomial(Rc[i, j, k], delta)

#E sampling
incubated[, , ] <- if(E[i, j, k] <= 0) 0 else Binomial(E[i, j, k], incubation_rate)

#I sampling
into_I[, , ] <- if(incubated[i, j, k] <= 0) 0 else Binomial(incubated[i, j, k], 1 - prop_severe[i, j, k])
into_Is[, , ] <- incubated[i, j, k] - into_I[i, j, k]
recovered_I_to_R[, , ] <- if(I[i, j, k] <= 0) 0 else Binomial(I[i, j, k], recovery_rate)

#Is sampling
recovered_from_Is[, , ] <- if(Is[i, j, k] <= 0) 0 else Binomial(Is[i, j, k], severe_recovery_rate)
recovered_Is_to_R[, , ] <- if(recovered_from_Is[i, j, k] <= 0) 0 else Binomial(recovered_from_Is[i, j, k], prop_complications)
recovered_Is_to_Rc[, , ] <- recovered_from_Is[i, j, k] - recovered_Is_to_R[i, j, k]

# Add in births and aging
aging_into_S[1, 1, ] <- Births[k]

aging_into_S[2:n_age, , ] <- if(S[i - 1, j, k] <= 0) 0 else Binomial(S[i - 1, j, k], aging_rate[i-1])
aging_out_of_S[1:(n_age - 1), , ] <- if(S[i, j, k] <= 0) 0 else Binomial(S[i, j, k], aging_rate[i])

aging_into_E[2:n_age, , ] <- if(E[i - 1, j, k] <= 0) 0 else Binomial(E[i - 1, j, k], aging_rate[i-1])
aging_out_of_E[1:(n_age - 1), , ] <- if(E[i, j, k] <= 0) 0 else Binomial(E[i, j, k], aging_rate[i])

aging_into_I[2:n_age, , ] <- if(I[i - 1, j, k] <= 0) 0 else Binomial(I[i - 1, j, k], aging_rate[i-1])
aging_out_of_I[1:(n_age - 1), , ] <- if(I[i, j, k] <= 0) 0 else Binomial(I[i, j, k], aging_rate[i])

aging_into_R[2:n_age, , ] <- if(R[i - 1, j, k] <= 0) 0 else Binomial(R[i - 1, j, k], aging_rate[i-1])
aging_out_of_R[1:(n_age - 1), , ] <- if(R[i, j, k] <= 0) 0 else Binomial(R[i, j, k], aging_rate[i])

aging_into_Is[2:n_age, , ] <- if(Is[i - 1, j, k] <= 0) 0 else Binomial(Is[i - 1, j, k], aging_rate[i-1])
aging_out_of_Is[1:(n_age - 1), , ] <- if(Is[i, j, k] <= 0) 0 else Binomial(Is[i, j, k], aging_rate[i])

aging_into_Rc[2:n_age, , ] <- if(Rc[i - 1, j, k] <= 0) 0 else Binomial(Rc[i - 1, j, k], aging_rate[i-1])
aging_out_of_Rc[1:(n_age - 1), , ] <- if(Rc[i, j, k] <= 0) 0 else Binomial(Rc[i, j, k], aging_rate[i])
...
vaccinating_into_S[, 2:n_vacc, ] <- if(S[i, j - 1, k] <= 0) 0 else Binomial(S[i, j - 1, k] , vaccination_prop[i, j - 1, k])
vaccinating_out_of_S[, 1:(n_vacc - 1), ] <- if(S[i, j, k] <= 0) 0 else Binomial(S[i, j, k] , vaccination_prop[i, j, k])

# Exposed vaccination
vaccinating_into_E[, 1, ] <- if(E[i, j, k] <= 0) 0 else Binomial(E[i, j, k] , 0)
vaccinating_into_E[, 2:n_vacc , ] <- if(E[i, j - 1, k] <= 0) 0 else Binomial(E[i, j - 1, k] , vaccination_prop[i, j - 1, k])
vaccinating_out_of_E[, 1:(n_vacc - 1), ] <- if(E[i, j , k] <= 0) 0 else Binomial(E[i, j , k] , vaccination_prop[i, j, k])

# Infectious vaccination
vaccinating_into_I[, 1, ] <- if(I[i, j, k] <= 0) 0 else Binomial(I[i, j, k] , 0)
vaccinating_into_I[, 2:n_vacc , ] <- if(I[i, j - 1, k] <= 0) 0 else Binomial(I[i, j - 1, k] , vaccination_prop[i, j - 1, k])
vaccinating_out_of_I[,1:(n_vacc - 1) , ] <- if(I[i, j, k] <= 0) 0 else Binomial(I[i, j, k] , vaccination_prop[i, j, k])

# Recovered vaccination
vaccinating_into_R[, 1, ] <- if(R[i, j, k] <= 0) 0 else Binomial(R[i, j, k] , 0)
vaccinating_into_R[, 2:n_vacc , ] <- if(R[i, j - 1, k] <= 0) 0 else Binomial(R[i, j - 1, k] , vaccination_prop[i, j - 1, k])
vaccinating_out_of_R[, 1:(n_vacc - 1) , ] <- if(R[i, j, k] <= 0) 0 else Binomial(R[i, j, k] , vaccination_prop[i, j, k])

# Infectious severe vaccination
vaccinating_into_Is[, 1, ] <- if(Is[i, j, k] <= 0) 0 else Binomial(Is[i, j, k] , 0)
vaccinating_into_Is[, 2:n_vacc , ] <- if(Is[i, j - 1, k] <= 0) 0 else Binomial(Is[i, j - 1, k] , vaccination_prop[i, j - 1, k])
vaccinating_out_of_Is[, 1:(n_vacc - 1) , ] <- if(Is[i, j, k] <= 0) 0 else Binomial(Is[i, j, k] , vaccination_prop[i, j, k])

# Recovered complications vaccination
vaccinating_into_Rc[, 1, ] <- if(Rc[i, j, k] <= 0) 0 else Binomial(Rc[i, j, k] , 0)
vaccinating_into_Rc[, 2:n_vacc , ] <- if(Rc[i, j - 1, k] <= 0) 0 else Binomial(Rc[i, j - 1, k] , vaccination_prop[i, j - 1, k])
vaccinating_out_of_Rc[, 1:(n_vacc - 1) , ] <- if(Rc[i, j, k] <= 0) 0 else Binomial(Rc[i, j, k] , vaccination_prop[i, j, k])
...
Npop_background_death[, ] <- Binomial(Npop_age_vulnerable[i, j], background_death[i, j])
...
Births[] <-  if(reproductive_population[i] <= 0) 0 else if(simp_birth == 1) Binomial(reproductive_population[i], birth_rate[i]) else Binomial(reproductive_population[i], birth_int[i])
